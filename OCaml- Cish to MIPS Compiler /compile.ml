...
let compile_func (f:Ast.funcsig) : Mips.inst list =
  ...
  let rec compile (p:Ast.program) : result =
    let rec compile_code (prog:Ast.program) : Mips.inst list =
      ...
      | [] -> []
      | (Ast.Fn(f))::tl ->
        if Varmap.member !funcmap (mangle f.Ast.name) then
          error "Function name collision\n"
        else
          funcmap := Varmap.insert_var !funcmap (mangle f.Ast.name) (List.length f.Ast.args);
          compile_func f @ compile_code tl
    in
